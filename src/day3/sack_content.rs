...
pub struct SackContent(Vec<Item>, Vec<Item>);
...
impl FromStr for SackContent {
    ...
    fn from_str(input_str: &str) -> Result<Self> {
        ...
        let sack = SackContent(part1?, part2?);
...
mod tests {
    ...
    #[test]
    fn test_parse() {
        let sack_content = "npbB".parse::<SackContent>().unwrap();
        assert_eq!(
            &vec![
    ...
    #[test]
    fn test_find_dup() {
        let sack_content = "helloHELLo".parse::<SackContent>().unwrap();
